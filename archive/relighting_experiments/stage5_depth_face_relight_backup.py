"""Archived local backup retained for research history only."""
...
def refine_loaded_person_mask(mask: np.ndarray) -> np.ndarray:
    ...
    mask = blur01(mask, scaled_kernel(5, h, w))
    mask = smoothstep(0.20, 0.70, mask)

    binary = (mask > 0.35).astype(np.uint8)

    close_k = scaled_size(13, h, w, min_value=5)
    ...
    binary = binary_keep_largest_components(
        binary=binary,
...
def build_or_load_person_matte(
    ...
    if person_mask_path is not None:
        ...
        person_matte = refine_loaded_person_mask(raw_mask)
        return person_matte, skin, True

    person_matte = build_fallback_person_matte(image_srgb, skin)
    return person_matte, skin, False
...
def relight_person_only(
    ...
    person_matte, skin, used_external_mask = build_or_load_person_matte(
        image_srgb=image_srgb,
        person_mask_path=person_mask_path,
    )

    confident_matte = build_confident_person_matte(person_matte)

    shadow = build_shadow_mask(image_srgb, image_linear)
    shadow_person = shadow * person_matte
    ...
    spec_mask *= person_matte

    physical += specular_strength * spec[:, :, None] * spec_mask[:, :, None]
    physical = np.clip(physical, 0.0, 1.0)

    background = 1.0 - person_matte

    relight_strength = person_matte * strength

    # Keep this 0.00 in normal use. This exists only for experiments.
    ...
    relight_strength = blur01(relight_strength, scaled_kernel(23, h, w))

    # Important: Gaussian blur can leak outside the person.
    # This clamps the relighting map back to the final person matte.
    relight_strength *= person_matte
    ...
    debug = {
        ...
        "background_restored_area": 1.0 - person_matte,
        "skin": skin,
        "shadow": shadow,
        "shadow_person": shadow_person,
        "normal_edge": normal_edge,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Sunit Stage 5 V7: person-locked relighting with robust matte support."
    )
    ...
    parser.add_argument(
        "--person-mask",
        default=None,
        help="Recommended. Real person segmentation mask. White=person, black=background.",
    )
...
def save_debug_maps(output_path: str | Path, debug: dict[str, np.ndarray]) -> None:
    ...
    print("Most important: person_matte_FINAL_BLEND.png")
    print("The gate/door/background should be black in that matte.")
...
def main() -> None:
    ...
    print(f"Saved Stage 5 V7 output to: {args.output}")
